...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">欢迎使用SillyLi表白墙程序安装向导</h3>
	</div>
	<div class="panel-body">
		<p><iframe src="https://yun.sillyli.com/forlover/info.php" style="width:100%;height:465px;"></iframe></p>
		<?php if($installed){ ?>
...
if(defined("SAE_ACCESSKEY") || $_GET['jump']==1){
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from sillyli_config where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你安装过SillyLi表白墙程序安装向导</div>
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
			if(DB::query("select * from qiu_config where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过SillyLi表白墙程序安装向导</div>
				<div class="list-group-item">
					<a href="?do=6" class="btn btn-block btn-info">跳过安装</a>
				</div>
				<div class="list-group-item">
...
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
    ...
    $sql=str_replace("sillylistuptimePOIJ7892BXS",time(),$sql);
